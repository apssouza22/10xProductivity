...
- **Credentials:** API-token tools load secrets from `.env`. Browser-session tools keep auth in the shared profile at `~/.browser_automation/agent_profile/` — use `session_request.py` for API calls; only instance URLs belong in `.env`.
