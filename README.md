...
| Operations and observability | [PagerDuty](tool_connections/pagerduty/setup.md), [Grafana](tool_connections/grafana/setup.md), [Langfuse](tool_connections/langfuse/setup.md), [Datadog](tool_connections/datadog/setup.md), etc. | Prepare incident context, investigate LLM/agent traces, connect alerts to owners and tickets, and summarize operational follow-ups |
...
The trigger uses the shared browser profile at `~/.browser_automation/agent_profile/` (and `SLACK_WORKSPACE_URL` from `.env`).
