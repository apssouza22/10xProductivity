...
Slack is accessed through a browser session stored in `~/.browser_automation/agent_profile/`. No API token page exists. No admin approval needed.
...
The script opens a Chromium window. Sign in through the browser if prompted. The session is saved to `~/.browser_automation/agent_profile/` — nothing auth-related goes to `.env`.
...
That uses `SLACK_ACME_WORKSPACE_URL` in `.env` to target the acme workspace (same shared profile at `~/.browser_automation/agent_profile/`).
