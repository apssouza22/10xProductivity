...
Grafana is accessed through a browser session in `~/.browser_automation/agent_profile/`. No API token page needed for instances that support web login.
...
Sign in through the browser if prompted. Session is saved to `~/.browser_automation/agent_profile/`.
