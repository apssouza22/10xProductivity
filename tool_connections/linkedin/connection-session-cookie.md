...
LinkedIn for personal use: read messages, reply, make posts, look up your own profile. No developer app required. Session cookies (`li_at`, `JSESSIONID`) live in `~/.browser_automation/agent_profile/` — not in `.env`.
...
**Important:** Always use the shared profile (`~/.browser_automation/agent_profile/`) so LinkedIn doesn't trigger 2FA on every run.
...
- **Persistent profile required:** `~/.browser_automation/agent_profile/` — never delete this folder or LinkedIn will 2FA again on next run.
